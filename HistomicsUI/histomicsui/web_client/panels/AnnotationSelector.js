...
var AnnotationSelector = Panel.extend({
    ...
    editAnnotationMetadata(evt) {
        ...
        const model = this.collection.get(id);
        this.listenToOnce(
            showSaveAnnotationDialog(model, {title: '修改标注', name:"标注名称",namePlaceholder:"输入您的标注的名称", description:"标注说明",descriptionPlaceHolder:"输入可选的描述",conDName:"创建日期",updDName:'更新日期',uidDName:"唯一ID",gvDName:"全球版",permDName:"编辑权限" ,saveButtonName:"保存",cancelButtonName:"取消", viewer: this.viewer}),
            'g:submit',
            () => {
                if (model.get('displayed')) {
    ...
    createAnnotation(evt) {
        ...
        this.listenToOnce(
            showSaveAnnotationDialog(model, {title: '创建标注', name:"标注名称",namePlaceholder:"输入您的标注的名称", description:"标注说明",descriptionPlaceHolder:"输入可选的描述",saveButtonName:"保存",cancelButtonName:"取消" }),
            'g:submit',
            () => {
